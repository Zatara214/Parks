...
- [x] **Filter rides by land — done 2026-09-15.** A chip row on the Rides tab, from
      `domain/ParkLands.kt` (generated by `tools/park-lands.py`). The note below was right
      that `/children` carries no land: OSM maps them as `place=locality` areas, so the land
      comes from the coordinate every ride already has. **93% of rides place into a land**
      (147/158): Magic Kingdom, Hollywood Studios, Epic Universe and Islands of Adventure at
      100%, EPCOT 97%, Universal Studios 79% (Hogwarts Express platforms and Halloween
      Horror Nights houses belong to no themed land), Animal Kingdom 73% (Discovery Island
      is unmapped). The chip row is hidden entirely for a park with no mapped lands.
